...
└── AiCollabDocs/       # 개발 문서 (GitHub Pages 배포 제외 가능)
    └── Specification/
        └── (이 문서)
```
...
1. GitHub에 `DayPick` 리포지토리 생성
2. 소스 파일 push (`index.html`, `manifest.json`, `sw.js`, `icons/`)
3. Settings → Pages → Source: `main` branch, `/ (root)` 선택
4. `https://{username}.github.io/DayPick/` 으로 접속 가능
...
| 단계 | 내용 |
|------|------|
| 1단계 | 핵심 기능 구현 (index.html - 날짜 목록 + 복사) |
| 2단계 | PWA 설정 (manifest.json + sw.js) |
| 3단계 | 아이콘 제작 |
| 4단계 | GitHub Pages 배포 및 스마트폰 테스트 |
